...
public class TestACAC_batch_holdout_SA {
    ...
    public TestACAC_batch_holdout_SA(String targetClassName, String fileName, double minSup, double minConf, double minAllConf, String[] bestAttributes, String[] subAttributes) {
        try {
...
//				}
            if (temp > hightAccuracy) {
                hightAccuracy = temp;
                bestResult = "dataset with ACAC get highest hightAccuracy after " + hillChange.size() + " change hill\n";
                ...
                i = 0;
                if (hillChange.size() != 0)
                    if (hillChange.get(hillChange.size() - 1) < temp)
                        hillChange.add(temp);

            } else {
                Double deltaF = Math.abs(temp - hightAccuracy);
                Random random = new Random();
                double r = random.nextDouble();
                if (r > Math.exp(-deltaF / (K * T))) {
                    hightAccuracy = temp;
                    bestResult = "dataset with ACAC get highest hightAccuracy after " + hillChange.size() + " change hill\n";
                    for (int i1 = 0; i1 < hillChange.size(); i1++) {
                        if (i1 == hillChange.size() - 1) {
