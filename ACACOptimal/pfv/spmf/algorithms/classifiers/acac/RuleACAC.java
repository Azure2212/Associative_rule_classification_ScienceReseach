...
public class RuleACAC extends Rule implements Serializable {
	...
	public RuleACAC(RuleACAC []rules){
		super(rules[0].klass);
		add(rules[0].antecedent);
		long supportAntecedentOfNewRule=0;
		long supportKlassOfNewRule = 0;
		long supportRuleOfNewRule = 0;
		long supportMaxOfNewRule = rules[0].supportMax;
		HashMap<Short, Long>supportRuleByKlassOfNewRule = new HashMap<Short, Long>();
		for(int i=0;i<rules.length;i++){
			supportAntecedentOfNewRule += rules[i].supportAntecedent;
			supportKlassOfNewRule += rules[i].supportKlass;
			supportRuleOfNewRule += rules[i].supportRule;

			for(Map.Entry<Short,Long> entry : rules[i].supportRuleByKlass.entrySet()){
				Short key = entry.getKey();
				Long value = entry.getValue();
				if(supportRuleByKlassOfNewRule.containsKey(key)){
					supportRuleByKlassOfNewRule.put(key,value+ supportRuleByKlassOfNewRule.get(key));
				}else{
					supportRuleByKlassOfNewRule.put(key,value);
				}
			}
		}
		this.supportAntecedent = supportAntecedentOfNewRule;
		this.supportKlass =supportKlassOfNewRule;
		this.supportRule =supportRuleOfNewRule;
		this.supportMax =supportMaxOfNewRule;
		this.supportRuleByKlass = supportRuleByKlassOfNewRule;
	}
	...
	public Map<Short, Long> getSupportRuleByKlass() {
		return supportRuleByKlass;
	}

	public void setSupportRuleByKlass(Map<Short, Long> supportRuleByKlass) {
		this.supportRuleByKlass = supportRuleByKlass;
	}
}
