 ...
 * @see AlgoACAC
 */
public class AprioriForACAC4Spark {

    /**
     * Default constructor
     */
    public AprioriForACAC4Spark() {
        // empty
    }
     ...
     * threshold
     */
    public List<RuleACAC> run(Dataset dataset, double minSup, double minConf, double minAllConf, List<RuleACAC> allRules) {
     ...
     */
    public List<RuleACAC> generateAndTestCandidateSizeK(Dataset dataset, double minConf, double minAllConf,
                                                        long minSupRelative, List<RuleACAC> rules, List<RuleACAC> level, List<RuleACAC> otherFragmentLevel) {
        // Store the rules of size k-1 in a variable
        List<RuleACAC> previousLevel = GenerateNewListACACRules(level, otherFragmentLevel);
        // add rule in other frament level
       // System.out.println("vo roi ne::"+previousLevel.size());
...
//								&& !level.contains(candidate)) {  //====  REMOVED BY PHILIPPE :  SEEMS UNECESSARY.    =====/
    ...
    private List<RuleACAC> GenerateNewListACACRules(List<RuleACAC> l1, List<RuleACAC> l2) {
        List<RuleACAC> newL = new ArrayList<>(l1);
        try {
            for (RuleACAC j : l2) {
                boolean flag = false;
                for (RuleACAC i : l1) {
