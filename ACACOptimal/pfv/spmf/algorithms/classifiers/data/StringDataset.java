...
import ca.pfv.spmf.algorithms.classifiers.acac.Spark4Distributing;
...
public class StringDataset extends Dataset implements Serializable {
     ...
     * @throws Exception
     */
    // update by Tran Anh Tuan
    public StringDataset(generateRules g, String path, String className) throws Exception {
        super();
        ...
        try {
            ...
            for(int i=0;i<g.tap_chinh.length;i++){
                if(g.tap_chinh[i].equals(className)){
                    System.out.println("coi ne");
                }
            }
            ...
            g.allAtributes = nameAttributes;

            int[] index_2_remove = g.removeHead(nameAttributes);

            nameAttributes = g.removeDateByindex(index_2_remove, nameAttributes);

            // System.out.println(index_2_remove.length);

//            for(int i=0;i<g.tap_chinh.length;i++){
//                System.out.print("'"+g.tap_chinh[i]+"', ");
//            }


            g.update_2_set(index_2_remove);
            ...
            while ((line = bufferedReader.readLine()) != null) {
                ...
                values = g.removeDateByindex(index_2_remove, values);
    ...
    //for FSAC
    public StringDataset(generateRules g, List<String[]> allData, String[] allAttributes, String className, String[] attributes) throws Exception {
        super();
        ...
        try {
            ...
            // Obtains the name of the attributes from the header
            String[] nameAttributes = allAttributes;
            //g.DogenerateRules();
...
//            }
            ...
            System.out.println("vô đây r"+allAttributes.length+"--"+allData.size()+"----"+metadataAttributes.size());
            System.out.println("class = "+klass);
            // Once all the different values have been saved, start iterating to find which
            // kind of attribute
            String[] klassValues = null;
    ...
    public StringDataset(Spark4Distributing s, String path, String className) throws Exception {
        super();
    ...
    public StringDataset(String path, String className) throws Exception {
        ...
        try {
            ...
            for (int i = 0; i < this.instances.size(); i++) {
                System.out.println(this.instances.get(i) + " ");
            }
            lines.clear();
    ...
    public StringDataset(String path, String className, int labelIndex) throws Exception {
        super();
    ...
    // for spark
    public StringDataset(String path, String className, int start, int end) throws Exception {
        super();
        ...
        try {
            ...
            /*for(int i=0;i<this.instances.size();i++){
                System.out.println(this.instances.get(i).getItems()[0]+" ");
            }*/
            List<Instance> newList = new ArrayList<>();
            for (int i = 0; i < this.instances.size(); i++) {


                Short[] fragmentInstance = new Short[end - start + 1 + 1];// contain label class
                //System.out.println("leng="+(end-start+1)+",start ="+start+", end="+end+", lengItem="+fragmentInstance.length);
                int index = 0;
                for (int j = start; j < start + fragmentInstance.length; j++) {
                    fragmentInstance[index++] = this.instances.get(i).getItems()[j];
                }

                fragmentInstance[fragmentInstance.length - 1] = this.instances.get(i).getItems()[this.instances.get(i).getItems().length - 1];


                newList.add(new Instance(fragmentInstance));
            }
