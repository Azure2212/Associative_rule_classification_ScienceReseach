...
//					System.out.println("TRAINING + " + j);
	...
	public static Dataset[] splitDatasetForHoldout2(Dataset dataset, int fragment) {
		int recordCount = dataset.getInstances().size();
		int groupSize = recordCount / fragment;
		int start =0, end = groupSize;
		Dataset[] datasets =new Dataset[fragment];
		datasets[0] = new VirtualDataset(dataset, start,end,"dataset");
		start= end ;
		end += groupSize;

		for(int i=1;i<fragment;i++){
			if(i==(fragment-1)){
				datasets[i] = new VirtualDataset(dataset, start, recordCount, "dataset");
			}else {
				datasets[i] = new VirtualDataset(dataset, start, end, "dataset");
				start = end;
				end += groupSize;
			}
