...
public class OverallResults{
	...
	public void printStats() {
		...
		accuracy= listMatrixOnTesting.get(0).getAccuracy();

		//System.out.println("coi accu= "+accuracy);
	}

	public String toPrintStats() {
		String rs="";
		rs+="=== MODEL TRAINING RESULTS ===\n";
		rs+=trainingMetricsToString(runtimeToTrain, memoryToTrain)+"\n";
		// training
		rs+="==== CLASSIFICATION RESULTS ON TRAINING DATA ====="+"\n";
		rs+=metricsToString(listMatrixOnTraining, runtimeOnTraining, memoryUsageOnTraining)+"\n";

		// testing
		rs+="==== CLASSIFICATION RESULTS ON TESTING DATA ====="+"\n";
		rs+=metricsToString(listMatrixOnTesting, runtimeOnTesting, memoryUsageOnTesting)+"\n";
		accuracy= listMatrixOnTesting.get(0).getAccuracy();
	...
	private String metricsToString(List<ConfusionMatrix> listMatrix,
		...
		for(int i =0; i< algorithmCount; i++) {
			...
			//accuracy=listMatrix.get(i).getAccuracy();
		}
